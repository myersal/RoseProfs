...
def borrowerOfBook(isbn):
		...
		for data in books:
			...
			for d in edge:
				borrower = client.command("select * from (TRAVERSE in('checked_out'), out('checked_out') from (SELECT * from book where isbn = " + str(isbn) + ") WHILE depth <= 2) WHERE @class = user")
				#borrower = client.command("select * from user where in_checked_out = " + books[0]._rid)
