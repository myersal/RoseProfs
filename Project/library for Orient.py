...
def recommendation(username):
		...
		result2 = client.command("Select * from (TRAVERSE out(rate_book), in(rate_book), out(book), out(user) from (SELECT * from user where username = '" + username + "') WHILE $depth <= 6) WHERE $depth = 3 and (@class = 'book' or @class = 'user')")
